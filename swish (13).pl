...
evento(caverna):-
    write('Voce encontrou um item poderoso na caverna!'), nl,
    pegarItem.

evento(vilaAbandonada):-
    write('Voce encontrou um abrigo na vila e pode descansar...'), nl,
    curar.
...
todos_eventos(L):-
    findall(Local, evento(Local, _), L).
...
%--Retorna um número para validações de quantidades máximas--%
total_visitas(T):-
    historico(H),
    length(H, T).

%---Retorna a true ou false para comparações--%
ja_visitou(Local):-
    historico(H),
    member(Local, H).